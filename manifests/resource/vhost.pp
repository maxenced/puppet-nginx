...
#     vhost.
#   [*ssl_certificate*]
#     Path to SSL certificate. Certificate won't be installed by this module.
#     Conflict with $ssl_cert.Default: undef
#
#   [*ssl_certificate_key*]
#     Path to SSL certificate key. Certificate won't be installed by this module.
#     Conflict with $ssl_key.Default: undef
#
#   [*manage_ssl_files*]
#     If set to true, this module will try to install certificates / keys in $ssl_dir.
#     Default: true
#
...
#   [*ssl_custom_config*]
#     If set to true, use the following ssl_* directives inside the vhost.
#     If set to false, it assumes you have a generic configuration for ssl.
#     Default: true
...
define nginx::resource::vhost (
  ...
  validate_bool($ssl_custom_config)
  ...
  validate_array($loc_proxy_set_header)
  ...
  if ($ssl == true) {
    if (($ssl_cert == undef) or ($ssl_key == undef)) and (($ssl_certificate == undef) or ($ssl_certificate_key == undef)){
  ...
  if $use_default_location == true {
    ...
    nginx::resource::location {"${name_sanitized}-default":
      ...
      proxy_set_header      => $loc_proxy_set_header,
      fastcgi               => $fastcgi,
  ...
  if ($ssl == true) {
    ...
    if ($manage_ssl_files) {
      ensure_resource('file', "${nginx::config::ssl_dir}/${cert}.crt", {
        owner  => $nginx::config::daemon_user,
        mode   => '0444',
        source => $ssl_cert,
        })
        ensure_resource('file', "${nginx::config::ssl_dir}/${cert}.key", {
          owner  => $nginx::config::daemon_user,
          mode   => '0440',
          source => $ssl_key,
          })
          if ($ssl_dhparam != undef) {
            ensure_resource('file', "${nginx::config::ssl_dir}/${cert}.dh.pem", {
              owner  => $nginx::config::daemon_user,
              mode   => '0440',
              source => $ssl_dhparam,
              })
          ...
          if ($ssl_stapling_file != undef) {
            ensure_resource('file', "${nginx::config::ssl_dir}/${cert}.ocsp.resp", {
              owner  => $nginx::config::daemon_user,
              mode   => '0440',
          ...
          if ($ssl_trusted_cert != undef) {
            ensure_resource('file', "${nginx::config::ssl_dir}/${cert}.trusted.crt", {
              owner  => $nginx::config::daemon_user,
              mode   => '0440',
